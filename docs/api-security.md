...
| **Tenant binding** | A credential acts in the organization it was minted for — never the browser's active-org cookie, and never beyond that org even when its owner is a global superuser. Out-of-scope resources return **404**, not 403. | resolver MACHINE-1 + scope cap MACHINE-2 ([design §3](./design-api-keys-and-tokens.md#3-caller-resolution)) |
...
| **Everything audited** | Issuance, denials, and mutations write audit events carrying the same `requestId` as the response's `x-request-id` header. The MCP transport and registration routes are the exception: they do not stamp the header yet ([observability §1](./observability.md#1-what-ships-today)), so their rows, such as `mcp.client.registered`, cannot be joined to a response. | `api.access.denied`, `token.issued`, `api_key.created`, … |
...
| A web app on one of your subdomains reusing platform sign-in | A **satellite app** on the SSO handoff (Options A/B), with **database credentials of its own** and a host **outside your `COOKIE_DOMAIN`** | Per-app cookie, per-app secret, ≤60s single-use handoff tokens. A compromise stays contained **only** when its database role cannot reach the primary's tables and its host is one the primary's cookie does not reach | [§8 below](#8-third-party-and-satellite-web-apps), [Satellite Apps Integration Guide §1.1](./integration-satellite-apps.md#11-when-a-or-b-is-actually-contained) |
...
1. **Create a dedicated service user per third party.** Never bind a third party's credential to a human's account: credentials inherit their owner's live permissions, so the human's role changes silently change the integration's authority — and off-boarding the human kills the integration. A per-party service user also gives you a clean audit line and an independent kill switch. Don't set or reset its password once its credentials are live: a completed password reset, or an admin setting the password, revokes every key and client that acts as that user (F-10, [§6](#6-revocation--incident-response)).
2. **Bind it to the right organization.** Credentials are org-bound at mint time (MACHINE-1) and confined to that org at use time (MACHINE-2) — a third party serving one tenant must get a credential minted in that tenant, and the binding holds even if the owning account is a platform superuser. If you need cross-tenant machine reach, that is not a thing a credential can be granted; use a human superadmin session, or one credential per tenant. **Check your existing org-less credentials.** A credential minted with no organization (possible only from an unbound superadmin session — `POST /api/v1/admin/oauth-clients` with `organizationId` omitted) resolves against its owner's *earliest* membership and is now confined to that one tenant. If such a credential's owner is a platform superuser it used to reach every tenant; it will now 404 / return empty lists elsewhere. Re-mint one credential per tenant it actually serves.
...
5. **Set an expiry — this is an operator step, not a default.** `API_KEY_DEFAULT_TTL_DAYS` is **unset out of the box**, so a key minted without an explicit `expiresInDays` lives until revoked. Set it (e.g. `API_KEY_DEFAULT_TTL_DAYS="365"`) on any deployment that issues keys to third parties. Review #202 asked for 365 to be the built-in default; it is knowingly deferred rather than applied, because flipping it would hand every existing deployment's next key an expiry its operator never chose and there is currently no `expiresInDays: null` opt-out — see the comment on the knob in `src/lib/env.ts`. Third-party keys should always expire: renewal is your periodic re-authorization checkpoint. **Recommended for every deployment, not just third-party ones** (review 2026-09-22 F-10): a user whose session is stolen for a minute can mint a key from `/api/v1/me/api-keys`. A password reset now revokes it (see [§6](#6-revocation--incident-response)), but only if the owner notices and resets. A default TTL bounds the key's life when nobody does.
...
| Every credential of a compromised **account** | The owner completes a **password reset** (forgot password), or an admin **sets the password** (`POST /api/administrator/users/{id}/password`, `mode: "set"`) | Every session of the account ends, including those it opened by impersonating someone. Every API key it owns and every OAuth client acting as it is **revoked**, so their JWTs die on their next request, and each gets an `api_key.revoked` / `oauth_client.revoked` audit row with `metadata.reason` `password_reset` / `password_set` (F-10). Credentials the account minted for **other** principals are not touched; find them by the `api_key.created` / `oauth_client.created` rows whose actor is the account. A key or client being created by one of the account's sessions or credentials at that moment is revoked too, or refused with a `401` (`credential_revoked` on `/api/v1`) and a `*.create_denied` audit row with reason `issuing_credential_revoked` (the issuance fence, [design §5.3](./design-api-keys-and-tokens.md#53-lifecycle)). Unlike a ban, this is permanent: new credentials must be issued. A self-service password **change** revokes no credential |
...
- **Creating a user places a person in the agent's org.** The `createUser` tool (and `POST /api/v1/users` from any credential) enrols the new user in the credential's organization, so it needs `admin.users.update` (or `admin.orgs.update`) as well as `admin.users.create`, and `admin.users.manage` to create an **active** user; a credential scoped to `admin.users.create` alone is refused with 403 (F-480, [Admin Manager §8.1](./admin-manager.md#81-users)). Grant the approval scope only to an integration that should admit people with no human approving them: a pending user stays pending until someone approves it, whatever the organization's sign-up policy.
...
- **Third-party or mixed-trust app → SSO handoff (Options A/B).** The satellite keeps its **own** session store, cookie (scoped to its own subdomain), and `BETTER_AUTH_SECRET`; there is **no shared secret at all** — handoff tokens are EdDSA-signed by the primary's private key and the satellite verifies them against the primary's public JWKS (`/api/sso/jwks.json`), and the only bridge is a **single-use, audience-bound, ≤60-second** token. A compromised satellite holds no signing capability, so it can forge no handoff. It is **contained**, impersonating no one on the primary or on sibling apps, only when two more things hold ([Integration Guide §1.1](./integration-satellite-apps.md#11-when-a-or-b-is-actually-contained)):
  - **database credentials of its own**: a Postgres role with no privileges on the primary's schema, either one on a separate cluster or project or a dedicated role that is not the primary's. With credentials that reach the primary's database it can write the auth tables (set a password, grant a role, read a reset token). A new database reached as the primary's role does not change that (roles are cluster-wide, and `CONNECT` is granted to `PUBLIC` by default), and neither does a different `DB_SCHEMA`, which is only a `search_path`.
  - **a host outside the primary's `COOKIE_DOMAIN`**, ideally a different registrable domain. Under it, every visitor's browser sends the satellite the primary's session cookie, which a compromised server replays on the primary. A distinct cookie prefix does not stop that.

  Miss either and the satellite is security-equivalent to Option C below, whatever its option letter: the reference forks' same-database setup, for one, is.
...
- **Set `CLIENT_IP_SOURCE` and `TRUSTED_PROXY_COUNT` correctly** ([Configuration](./configuration.md#choosing-the-client-ip-source)). By default (`CLIENT_IP_SOURCE=xff`) rate-limit keys derive the client IP a fixed number of hops from the right of `X-Forwarded-For` (a port suffix is stripped, a hop that is not an IP address counts as none, and an IPv6 client is keyed by its /64, F-16); a wrong count lets an attacker spoof their IP into someone else's bucket (too high) or rate-limits your proxy instead of clients (too low). That model needs an edge that **overwrites or appends to** the client's `X-Forwarded-For`. With **no proxy** in front, or one that only sets `X-Real-IP` or a CDN header, the client's own `X-Forwarded-For` is trusted and every request can claim a fresh address, which defeats every per-IP bucket (only the token endpoint's global floor still holds) and forges the IP on audit rows (F-17). Put an overwriting proxy in front, or name the header your edge sets (`CLIENT_IP_SOURCE=x-real-ip`, `cf-connecting-ip`, …).
- **The token endpoint is defended in depth**: a per-trusted-IP bucket and then a global floor before the body is read or anything is verified, and a per-credential bucket only once the credential has verified — so knowing a victim's public `client_id` cannot lock that client out, rotating ids cannot escape the IP bucket, and spoofed forwarded headers still hit the floor ([design §10.2](./design-api-keys-and-tokens.md#102-rate-limiting)). The floor is charged only for requests the IP bucket admitted, so one IP whose requests are being refused cannot drain it and lock out every tenant (F-18). MCP registration and the CSP report sink pair their per-IP bucket and global floor the same way. The two pre-auth buckets are **shared across instances** (a Postgres-backed bucket, `app_rate_limits`), so the floor really is deployment-wide on a multi-instance or serverless deployment; if the database is unreachable they fall back to a per-instance bucket and log a warning (`devresponsekit_rate_limit_shared_fallbacks_total` counts it) rather than failing open. The per-credential bucket is in-process — its fan-out is bounded by the credentials the caller holds.
...
- [ ] Set `CLIENT_IP_SOURCE` (and, for `xff`, `TRUSTED_PROXY_COUNT`) for your proxy/CDN topology, and never expose the app without a proxy that overwrites that header
...
- [ ] Its own `DATABASE_URL` + `BETTER_AUTH_SECRET` (A/B): the URL signs in as a role with no privileges on the primary's schema, on a separate cluster or as a dedicated role that is not the primary's (none of these counts: a new database under the primary's role, a role made in the Neon Console, which joins `neon_superuser`, or a different `DB_SCHEMA`); machine-API credential separate from SSO artifacts
- [ ] Its host outside the primary's `COOKIE_DOMAIN` (A/B), ideally on a different registrable domain
