...
- **Set `TRUSTED_PROXY_COUNT` correctly** ([Configuration](./configuration.md#reverse-proxy--limits)). Rate-limit keys derive the client IP a fixed number of hops from the right of `X-Forwarded-For` (a port suffix is stripped, a hop that is not an IP address counts as none, and an IPv6 client is keyed by its /64, F-16); a wrong value lets an attacker spoof their IP into someone else's bucket (too high) or rate-limits your proxy instead of clients (too low).
