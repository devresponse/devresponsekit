...
- **The token endpoint is defended in depth**: per-client/IP buckets plus a global floor that holds even when attackers rotate `client_id`s or spoof forwarded headers ([design §10.2](./design-api-keys-and-tokens.md#102-rate-limiting)).
