...
- **Tools are generated from the OpenAPI spec.** Each scoped `/api/v1` operation becomes one MCP tool (~18 today), carrying its required scope; a new scoped endpoint becomes a tool for free. `issueToken`, `getJwks`, and `getOpenApi` are excluded. An array argument (`sort`, `filter[…]`) is sent as one query parameter per value, and a value outside the spec's `enum` is refused (`-32602`) before the API is called (F-34). Header parameters are not tool arguments: `setUserStatus` runs without `If-Match`, and its description says so.
