...
- **Query** — `page` (1-indexed), `pageSize`, `sort` (repeatable `field.asc` / `field.desc`, applied in order), `q` (case-insensitive search), and repeatable `filter[…]` exact-match filters. Several values are **repeated parameters** (`?filter[status]=blocked&filter[status]=suspended` matches either; `?sort=created_at.desc&sort=status.asc`), never a comma-joined list: a comma is part of the value.
- **Strict on v1** — every `/api/v1` list refuses what it cannot apply with a `400 invalid_request` problem whose `detail` names the accepted values: a `filter[…]` the operation does not list, an empty filter value or one outside the spec's `enum`, a `sort` on an unlisted field or with a direction other than `asc` / `desc`, a `q` on a list that does not search, and a single-valued parameter (`page`, `pageSize`, `q`, `status`, `appUserId`) given twice (F-34). Only `/users` searches. The credential listings (`/admin/api-keys`, `/admin/oauth-clients`) take no `sort`, `q` or `filter[…]` at all, only `status` (one of its `enum`) and, on api-keys, `appUserId`. The console API keeps the lenient contract of [Admin Manager §7](./admin-manager.md#7-list-queries-pagination-and-row-actions) and drops those instead.
...
| `/api/v1/users` | GET / POST | `admin.users.read` / `.create` | User administration. POST with the Better Auth `role: "admin"` also needs cross-org reach, which only a superadmin's cookie session has: every API key and JWT is bound to one org ([MACHINE-2](./design-api-keys-and-tokens.md#3-caller-resolution)), so a bearer caller always gets **403** `forbidden` for it (F-13). A caller without cross-org reach (every API key and JWT, and an org admin's session) enrols the new user in the org it acts in, in the same transaction, with a membership whose status is the user's `initialAppStatus` (default `pending_approval`; approving the user activates both). A user's tenant is its membership, so without it that caller got **404** on every follow-up on `/users/{id}`. A superadmin's cookie session still creates the user in no org; place it with `POST /api/administrator/users/{id}/memberships`. The MCP `createUser` tool calls this route, so an agent's user joins the agent's org. That enrolment is a membership add, and an `active` one an approval, so such a caller also needs `admin.users.update` or `admin.orgs.update`, plus `admin.users.manage` to pass `initialAppStatus: "active"`, each held as a permission **and** granted as a scope. A key or agent scoped to `admin.users.create` alone gets **403** `forbidden`, with a `detail` naming what is missing and an `admin.user.create_denied` audit row, before anything is created; so does an address whose email domain a superadmin bound to another organization (F-480, [Admin Manager §8.1](./admin-manager.md#81-users)). [`docs/openapi.json`](./openapi.json) lists the two scope sets as `createUser`'s security alternatives and states the rest in the operation's description, which the MCP tool carries. A pending user created this way is not activated by the org's sign-up policy when it signs in: approve it with `POST /api/v1/users/{id}/status` `approve`, which activates the user and the membership. **Behaviour change:** a client that used to create the user and then enrol it with `POST /api/administrator/users/{id}/memberships` in the same org now gets **409** `membership_exists` from the second call, since the create already holds that membership; approve the user instead (or change the membership's status with `PATCH …/memberships`). An optional `name` must be at most 200 characters with no control, line-break or invisible formatting characters, or the answer is **400** `invalid_request` (F-21, [Form Validation](./form-validation.md#a-persons-name)). An address that already has an account is **409** `conflict`, including one the identity provider holds with no user row and the loser of two concurrent creates; a failure to store the new user is **502** (F-30) |
...
This is the console surface (users, roles, permissions, groups, organizations, memberships, sign-up policy, invitations, enterprise apps, API keys, email, MCP agents, audit, CSV export). It is **internal tooling that mirrors the admin console — not a public/integration API**; prefer the v1 surface for integrations. Every endpoint accepts a cookie session (+ `Origin` on mutations) or a scope-bound bearer credential and requires the noted permission; mutations are rate-limited (`429` with `Retry-After` / `retryAfter`) and audited. That includes the user actions backed by Better Auth (create, ban, unban, soft-delete, restore, password, sessions), which a bearer caller could not complete before F-13 (they answered `502`). The exceptions are **cookie-session only**:

- The Better Auth platform role. `POST /users/[id]/role`, and `POST /users` with `role: "admin"`, need cross-org reach. Every API key and JWT is bound to one org ([MACHINE-2](./design-api-keys-and-tokens.md#3-caller-resolution)), so a bearer caller gets `403` there (the role route refused one before F-13 too); only a superadmin's cookie session may set the role.
- The two impersonation operations. `POST /users/[id]/impersonate` acts on the caller's own session cookie and hands back a new one, so a caller without that cookie session gets `403`; `DELETE /users/[id]/impersonate` bypasses the permission guard by design.
...
- **Rate limits.** `launch` is throttled per principal (session user id, or trusted client IP while signed out); `consume` GET and POST are throttled per trusted client IP. Both use the standard 30-burst / 1 per second budget and answer `429 rate_limited` with a `Retry-After` header before any `sso.*` audit row is written. The per-IP budgets (consume, and a signed-out launch) are shared by every instance, with no deployment-wide floor behind them, so traffic from other client IPs cannot refuse your handoff (F-19; [Architecture → Rate limiting](./architecture.md#rate-limiting)).
...
- **Tools are generated from the OpenAPI spec.** Each scoped `/api/v1` operation becomes one MCP tool (~18 today), carrying every scope its `security` requires (all of one set, or any one of several: `createUser` needs `admin.users.create` with `admin.users.update` or `admin.orgs.update`, F-480) and the operation's description when it has one; a new scoped endpoint becomes a tool for free. `issueToken`, `getJwks`, and `getOpenApi` are excluded. An array argument (`sort`, `filter[…]`) is sent as one query parameter per value, and a value outside the spec's `enum` is refused (`-32602`) before the API is called (F-34). Header parameters are not tool arguments: `setUserStatus` runs without `If-Match`, and its description says so.
