...
- **Plugins:** the built-in `admin` plugin (ban / impersonate / session management), a server-only `ssoSession` plugin (subdomain SSO), a `response-floor` plugin (`src/lib/auth-response-floor.ts`, F-20) that holds HTTP responses on sign-up, the password-reset request and resend-verification to at least 500 ms so response time does not reveal which addresses have accounts, and `nextCookies` (must be last).
