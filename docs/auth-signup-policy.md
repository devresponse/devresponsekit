...
5. otherwise the **default organization**, governed by its policy. That is the one organization flagged `is_default` (Administrator → Organizations → *organization* → **Settings** → *Set as default organization*), **whatever its slug** (F-40). The slug is only a name: renaming the default org changes nothing about where sign-ups land, and ticking the checkbox on another organization moves the default there (it is cleared on the previous one in the same transaction), so new sign-ups land in the new default from then on. The default can be moved but never unset (the API refuses with 409 `organization_is_default`). If no organization is flagged default at all (the admin write paths cannot produce that: the flag can only be moved, and a flagged org cannot be deleted, both checked under the same lock, so only a direct database edit can), the sign-up is **not provisioned**: nothing is created, the error is logged with the fix, and the account has no access until an operator flags a default; the verification decision fails closed meanwhile. Before F-40 this step (and the sign-up-time policy lookup) found the org by the slug `default`, so after a rename the next such sign-up created a second, adminless "Default Organization" and ran under the platform policy instead of the real default org's.

**Microsoft and Google tenant claims are deliberately not read** (review #38). The Entra tenant id (`tid`) and the Google hosted domain (`hd`) are **not** consulted at step 3, so a Workspace or Entra sign-up that matches no invitation, no `?org=` hint and no email-domain binding lands in the default organization — never in an organization derived from a raw tenant id. The resolver branches for those claims existed but were never reachable (no call site ever supplied the claims) and were removed rather than wired up, because switching them on would have silently repointed every existing Workspace/Entra sign-up out of the default organization and spawned GUID-slugged organizations from tenant ids. To place those sign-ups deliberately, use the admin-curated paths: an invitation (§6), an organization-scoped hint (§7), or the `app_provider_organizations` domain binding of step 4. See `src/lib/provider-organization-resolver.ts` for the same statement next to the code.
...
- **The accept lands in the inviting organization (F-33).** On success, `POST /api/invitations/accept` sets the `active_org` cookie to the inviting org. It uses the same attributes as the org switcher and records an `account.active_organization.changed` audit event with `source: "invitation_accepted"`. An existing member of another org is therefore taken to the org they just joined instead of their earlier one. The cookie is set only when the membership there is now **active**: accepting does not lift an existing `blocked` or `suspended` membership, and the cookie never names an org the user cannot enter. It is never set for an impersonated session. A new account that accepts through sign-up needs no cookie: the session prefers an active membership over any other, so a stray `pending_approval` membership elsewhere (in the default organization, say) never outranks the invited org ([Administrator console §8.3](./admin-manager.md#83-memberships)).
