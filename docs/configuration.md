...
- Server-side variables are validated in `src/lib/env.ts` — several features **fail fast at boot** if misconfigured (e.g. `API_JWT_ENABLED` without a signing key, or an email provider without its credentials). The boot error names each invalid variable **and the rule it broke** (never its value), so a variable held in write-only storage can still be diagnosed.
