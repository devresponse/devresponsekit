...
- Server-side variables are validated in `src/lib/env.ts` — several features **fail fast at boot** if misconfigured (e.g. `API_JWT_ENABLED` without a signing key, or an email provider without its credentials). "At boot" is literal (F-26): the Node boot hook (`register()` in `src/instrumentation.ts`) parses the whole schema before the server serves a request, so `next start` and the Docker image exit with code 1, and on Vercel every server-rendered page and API route of that deployment fails, readiness included. Before F-26 the parse ran on the first request that needed a value, so an instance with an invalid variable started, passed its health checks and then answered 500 on every page that touched auth. `next build` still never fails on the environment (it parses placeholders, [Deployment §3](./deployment.md#3-vercel-project--environment)). The boot error names each invalid variable **and the rule it broke** (never its value), so a variable held in write-only storage can still be diagnosed. `GET /api/health/ready` re-checks the schema and answers **503 `config_invalid`**, logging only the invalid variable names (`kind: "config-invalid"`), for a runtime where the hook did not stop the server.
- **Origin-valued variables are checked at boot (F-22).** `BETTER_AUTH_URL`, `SSO_HANDOFF_ISSUER`, `API_JWT_ISSUER`, `MCP_DISPATCH_BASE_URL`, `MAILGUN_BASE_URL` and every `ADMIN_TRUSTED_ORIGINS` entry must be an `http://` or `https://` **origin**: scheme, host and optional port, with no path, query, fragment or credentials. `httsp://…`, `ftp://…` and `https:/host` all fail. Under `NODE_ENV=production` (which includes every Vercel preview) they must use `https://` unless the host is loopback (`localhost`, `127.0.0.1`, `[::1]`); that exception is what keeps `next build` and CI's `next start` green. The two issuers are compared as exact strings by whoever verifies the token, so `SSO_HANDOFF_ISSUER` and `API_JWT_ISSUER` must be written exactly as the origin, in lowercase with **no trailing slash**. The others tolerate a trailing slash, because the app trims it, or re-parses the value, wherever it builds a URL from them. One use is as written: with `API_JWT_ISSUER` unset, `BETTER_AUTH_URL` becomes the JWT `iss` character for character, so leave its slash off too if anything outside this app verifies those tokens. `COOKIE_DOMAIN`, when set, must be `BETTER_AUTH_URL`'s host or a parent domain of it, must not be a public suffix, and must be written as `example.com` or `.example.com` with no trailing dot, because it reaches the browser exactly as written. The Ed25519 private keys (`SSO_HANDOFF_*` and `API_JWT_*`) are shape-checked by the schema (`x` and `d` each 43 unpadded base64url characters), and the Node boot hook (`register()` in `src/instrumentation.ts`) imports each one and checks that `x` is `d`'s public half, so a key that cannot be used stops the server from starting.
...
| `BETTER_AUTH_URL` | **yes** | Origin where `/api/auth/*` is reachable. Must match the browser origin. Also the base origin baked into links in outbound emails — verification, password reset, and invitation accept links. An http(s) origin, `https://` in production unless loopback (§1); a trailing slash is tolerated. |
| `ADMIN_TRUSTED_ORIGINS` | no | Extra comma-separated trusted origins for Better Auth and the admin origin guard (the app's own origin is always trusted). Each entry must be an http(s) origin, `https://` in production unless loopback (§1). |
| `COOKIE_DOMAIN` | no | Parent domain for the session cookie (e.g. `.devresponse.com`). **Unset = host-only cookie (per-app isolation — the safe default).** Set only for co-trusted shared-`auth`-schema satellites (Option C), on the primary **and** every satellite alike — see [Satellite Apps §5](./integration-satellite-apps.md#5-option-c-in-detail--shared-auth-schema). Once set, the browser sends this session cookie to **every** host under the domain, A/B handoff satellites included, so an A/B satellite there is no longer contained ([Satellite Apps §1.1](./integration-satellite-apps.md#11-when-a-or-b-is-actually-contained)). When set, it must be `BETTER_AUTH_URL`'s host or a parent domain of it, and a registrable domain rather than a public suffix such as `.com` or `.co.uk`. Write it as `example.com` or `.example.com`: one optional leading dot, no trailing dot (`example.com.` is a valid DNS name, but a browser drops the cookie), no surrounding spaces. The app refuses to boot otherwise: a browser silently drops such a cookie, so every sign-in would appear to succeed and sign nobody in. |
| `SESSION_ABSOLUTE_LIFETIME_HOURS` | no | **Absolute** session lifetime in hours, measured from session **creation** (ASVS V3 absolute timeout). **Unset = no cap — the shipped default, unchanged.** Sessions here are *rolling* (an 8-hour window refreshed after 15 minutes of activity), so an actively-used session — including a stolen cookie — never ages out by itself. Set e.g. `168` (7 days) or `336` (14 days) to force a re-authentication regardless of activity; a session past the cap is reported as absent by `getCurrentSession()` and its row is revoked, so browser guards redirect to sign-in and cookie-authenticated `/api/v1` calls 401. Max `8760`. Independently of this variable, an **impersonation** session is refused and revoked the same way one hour after it was created (F-08, docs/admin-manager.md §19). |
...
| `DATABASE_TEST_URL` | for tests | The database `pnpm test:db` runs against; the suites receive it as their `DATABASE_URL`. Unset or empty: they use `DATABASE_URL`. Either way a host that is not local is refused before a connection is opened ([Testing §3](./testing.md#the-db-backed-suites-database)). It needs the migrations, not a seed. The app never reads it. |
| `DB_TEST_ALLOW_REMOTE` | no | Set `1` to let `pnpm test:db` run against a database whose host is **not local** (`localhost` / `127.0.0.1` / `::1` / `0.0.0.0` / none). Off by default, and `CI` does not lift it: the suites create and delete users and organizations and briefly change the platform sign-up policy. Use it only for a disposable database. |
| `DB_SCHEMA` | no | Schema **all** tables (app + Better Auth) are deployed into. Default `auth`. Applied at the connection level as `search_path=<DB_SCHEMA>,public`; extensions (`pgcrypto`, `pg_trgm`) stay shared in `public`. Set a different value per deployment to keep their tables apart. It is a `search_path`, not a permission boundary: a deployment whose role can reach another schema reads and writes it with a qualified name, so separating deployments for security takes a role per deployment with no privileges on the others' schemas. A separate database is not enough on its own: roles are cluster-wide, so one reached as the same role is no boundary ([Satellite Apps §1.1](./integration-satellite-apps.md#11-when-a-or-b-is-actually-contained)). Must be a plain SQL identifier. |
...
> **Account linking requires a VERIFIED provider email.** When a social sign-in matches an existing local account by email, the accounts are linked only if the provider asserts the address is **verified** (`trustedProviders` is deliberately empty — no provider is exempt from this check, which blocks nOAuth-style takeover via attacker-controlled Entra tenants). Google and GitHub report verification for normal accounts, so linking just works. Entra ID often **omits** the `email_verified` claim, in which case a Microsoft sign-in cannot link into an existing email/password account and is rejected; the user can keep signing in with their password, and a tenant that needs Microsoft linking should emit a verified-email optional claim (e.g. `email_verified` / `verified_primary_email`). The **local** account's address must be proven too: an account created under a verification-waived sign-up policy, or by an org admin (not a superadmin) with a password, is refused provider linking until its owner completes a password reset (F-03, see [Sign-up Policy §9](./auth-signup-policy.md#9-security-notes)).

### Single Sign-On handoff

> **Required at boot for every deployment.** Despite the "SSO" name, `src/lib/env.ts` validates `SSO_HANDOFF_ISSUER`, `SSO_HANDOFF_AUDIENCE_PREFIX`, and `SSO_HANDOFF_APPLICATION_ID` **unconditionally** (`.min(1)`). The app **fails fast at boot** if any is missing — even on a deployment that never uses SSO. Set all three everywhere. When SSO is unused, the audience prefix and application id can be any placeholder, but `SSO_HANDOFF_ISSUER` must still be an http(s) origin (F-22): use this deployment's own `BETTER_AUTH_URL`. A non-URL placeholder such as `unused` now fails boot, because `/api/sso/consume` is mounted on every deployment and would otherwise 401 every handoff. The signing key (`SSO_HANDOFF_PRIVATE_KEY`) is **optional** and belongs on the **issuer only**.

> **Asymmetric since review #5.** Handoff tokens are **EdDSA (Ed25519)**-signed by the issuer's private JWK and verified by consumers against the issuer's public key set at **`GET <SSO_HANDOFF_ISSUER>/api/sso/jwks.json`**. A consumer (satellite) holds **no signing material** — the former fleet-wide symmetric `SSO_HANDOFF_JWT_SECRET` is gone, so a compromised satellite can forge no handoff token for its siblings. Whether it is contained otherwise depends on its database and cookie domain ([Satellite Apps §1.1](./integration-satellite-apps.md#11-when-a-or-b-is-actually-contained)).

| Variable | Required | Controls |
| --- | --- | --- |
| `SSO_HANDOFF_ISSUER` | **yes (at boot)** | `iss` claim of handoff tokens — the **issuer's origin URL** (e.g. `https://app.example.com`). Consumers fetch the issuer's public keys from `${SSO_HANDOFF_ISSUER}/api/sso/jwks.json`, so it must be a URL. Write it **exactly** as the origin: `https://` in production (loopback excepted), lowercase, no path and **no trailing slash**. The issuer stamps it into every token and each satellite compares `iss` with its own copy character for character, so `https://app.example.com/` or a typo such as `httsp://…` would break every handoff. Both fail boot (F-22), and the signer refuses to sign under such a value. |
...
| `SSO_HANDOFF_PRIVATE_KEY` | no (issuer only) | Ed25519 **private JWK** (JSON string) that signs handoff tokens (`alg: EdDSA`). Set it **only on the issuer**; consumers verify against the published JWKS. Unset ⇒ `/api/sso/launch` answers `503 sso_not_configured` (audited) while `/api/sso/consume` keeps working. **Checked at boot** when present: the schema requires a `kty: OKP`, `crv: Ed25519` JWK with a 32-byte `x` and `d`, and the Node boot hook imports it and requires `x` to be `d`'s public half, so a truncated or corrupted key fails boot rather than the first launch (F-22). **Must differ** from `API_JWT_PRIVATE_KEY` — the two are independent trust domains. |
| `SSO_HANDOFF_KID` | no | Fixed `kid` for the handoff key; defaults to the JWK thumbprint (so it changes with the key material). |
| `SSO_HANDOFF_PREVIOUS_PRIVATE_KEY` / `SSO_HANDOFF_PREVIOUS_KID` | no | Zero-downtime rotation: the **previous** private key (public half stays published + accepted) while tokens minted under it expire (≤60s). See the rotation steps below. The key is checked at boot the same way as the current one. |
...
1. Set **`API_JWT_ENABLED=1`** *and* provide **`API_JWT_PRIVATE_KEY`**. The app **fails to boot** if the switch is on without a key (validated in `src/lib/env.ts`), so you cannot half-enable it. It also fails to boot when `API_JWT_PRIVATE_KEY` or `API_JWT_PREVIOUS_PRIVATE_KEY` is set but does not import as an Ed25519 private key (a truncated value, a stray quote, an `x` from another key). Before F-22 such a key booted, and then every token failed with a generic 401.
...
| `API_KEY_DEFAULT_TTL_DAYS` | Default key expiry (empty = never expire; UI warns). Recommended on every deployment, and essential if you issue keys to third parties: a key minted from a stolen session is revoked by a password reset only if the owner resets (F-10) — [api-security §3](./api-security.md#3-operator-playbook--granting-access-safely). The default stays unset, so choosing it is an operator decision. |
...
| `API_JWT_ISSUER` | `iss` claim (defaults to `BETTER_AUTH_URL`; an empty value counts as unset). When set, an http(s) origin written exactly, with **no trailing slash**, because verifiers compare `iss` as an exact string (§1). |
...
| `API_JWT_PRIVATE_KEY` | Ed25519 private JWK as a JSON string. **Required** when JWT enabled. Imported at boot when set. |
| `API_JWT_KID` | Key id (defaults to the JWK thumbprint). |
| `API_JWT_PREVIOUS_PRIVATE_KEY` | Verify-only previous signing key (the prior `API_JWT_PRIVATE_KEY`) kept during a rotation overlap — its public half stays in JWKS so tokens minted before the rotation keep verifying until they expire. Never used to mint; remove once that window drains. Imported at boot when set: a bad previous key breaks the whole key set, including tokens from the current key. |
...
| `MCP_DISPATCH_BASE_URL` | Origin the gateway self-calls when a tool invokes `/api/v1`. Default: `BETTER_AUTH_URL`. Point it at an origin that reaches the app **without** the edge proxy (e.g. `http://127.0.0.1:3000`) so the forwarded client IP below survives the hop. It must be an http(s) origin. In production it must be `https://` unless the host is loopback, because the agent's bearer rides on the hop, and a same-host hop can always use `127.0.0.1`. |
