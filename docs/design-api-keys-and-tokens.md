...
- **MACHINE-2 (bound reach).** MACHINE-1 pins *which* membership the credential resolves against; MACHINE-2 pins how far it can then see. `getUserAccessContext` expands a **global superuser** principal to the full superuser permission set on the bound path too (correctly — a bound superuser really does hold every capability inside its bound tenant), and the org-boundary helpers used to read that marker as "every org". A key minted in org A whose owner happened to be a superuser therefore administered the whole platform, and an org admin holding `admin.apikeys.manage` could mint exactly such a key on behalf of any superuser co-member. `resolveOrgScope` / `canAccessOrg` / `canAccessUser` now gate the bypass on `hasCrossOrgReach` (`src/lib/admin/access-scope.server.ts`): an **org-bound context is confined to its bound org**, and to nothing when it has none. `isSuperadmin` is unchanged and still reports the principal's true rank. Cookie sessions are untouched. Two consequences follow from the cap, and both are enforced rather than assumed:
  - **Layer 2 — on-behalf issuance.** All four paths that hand the ACTOR a one-time secret for a credential that authenticates as SOMEONE ELSE return **403** when that principal is a superuser and the acting caller is not: `POST /api/administrator/api-keys`, its `[id]/rotate` twin, `POST /api/v1/admin/oauth-clients` and ITS `[id]/rotate-secret` twin (`ownerOutranksActor`). "Is a superuser" is the principal's RANK (`userHoldsSuperuserGrant`), not its current authority: since F-09 a grant held in a suspended organization confers no authority, but it wakes when that organization is reactivated, so a credential minted or rotated in the meantime would then authenticate as a platform superuser. The two rotate paths were the softer targets — they carried the existing scopes forward verbatim and never ran the scope bound at all, which F-01 closed by routing them through the shared issuance rule (§7). Per **P1-1**, the "acting caller is a superadmin" exemption requires a COOKIE session (`grantedScopes === null`): `access.permissions` is the credential OWNER's held set, not the credential's authority, so a superuser-owned but narrowly-scoped key is *not* exempt.
  - **Impersonation is refused to a bound credential.** `POST /api/administrator/users/[id]/impersonate` returns **403** for any org-bound caller, and `targetOutranksActor` refuses an org-bound actor any global-superuser target. Both are RANK rules rather than permission-subset comparisons, because the subset comparison is vacuous for a bound superuser credential — its expanded set is the whole `ADMIN_PERMISSION_CATALOG`, a superset of every target's. Impersonation returns a *cookie* session, which is unbound by definition, so it is the one action that could convert a tenant-confined credential into an unconfined one.

An **org-less** credential (minted with `organization_id = null`, which only an unbound superadmin can create) is org-bound to the principal's earliest membership and scoped there — the same rule its non-superuser counterpart has always followed. There is no credential shape that reaches every tenant.
...
**Tenant confinement (F-01).** The surface is account-wide — every key the user holds, in every org — only for the owner on an ordinary cookie session. A **bearer** caller is confined to the one org its credential acts in, exactly like an impersonated session (IMP-1): listing is filtered to that org, and revoking or rotating a key in any other org answers `404`, the same as a key that is not the caller's (`tenantConfinement` / `isWithinTenant`, `src/lib/account/guard.server.ts`). A bearer rotation must also pass the issuance rule (§7) against the key's scopes, so a key scoped only `account.apikeys.manage` cannot rotate its owner's `admin.*` key and receive it. Without both, a credential bound to org A could list its owner's org-B keys and rotate one into a fresh org-B admin secret.
...
2. **At creation** — a credential can never be minted to out-scope its creator:
   ...
   - `ungrantableScopesForCaller(callerPermissions, callerGrantedScopes, requested)` closes the **bearer self-escalation** gap: a cookie caller (`callerGrantedScopes === null`) delegates with full user authority, but a **bearer** caller may delegate only scopes it already holds — so a narrowly-scoped key can never mint a broader one. On behalf of **another** principal, the issuance rule (item 3) additionally requires the issuer's **live permissions** to cover every scope (`scopes ∩ permissions`, F-05): a bearer grant can name more than its owner holds (an agent ceiling set above its service user's role, or a key whose owner was later downgraded) — inert at use, where the guards intersect with live permissions, but live if conferred on a credential for a co-member who does hold it. Re-issuing the issuer's **own** credential (a key rotating itself) is not bounded this way, since the excess stays inert.
3. **At every issuance, through one chokepoint (F-01).** `unissuableScopes` (`src/lib/api-auth/issuance.ts`) is the single rule every path that hands out a credential, or changes what one can do, runs before it writes. "Issuance" includes **rotation** (the successor carries the original scopes forward and the new secret goes to whoever asked) and **re-scoping an OAuth client** (the caller may already hold its secret). The rule is `ungrantableScopesForCaller` plus: **no account-WRITING scope (`account.apikeys.manage`, `account.profile.write`, `account.preferences.write`, or a wildcard over them) on a credential that authenticates as someone other than the issuer** — and an impersonated session never counts as the owner, since it only carries the borrowed identity. Those scopes act on the owner's own account, so an administrator must never hold them for another principal: an on-behalf key carrying `account.apikeys.manage` reached the owner's self-service key surface, and from there their keys in other tenants. `account.read` stays grantable on behalf — read-only introspection inside the credential's tenant, which a service principal's `/me` probe and an agent's `whoami` tool need. The paths: `POST /api/administrator/api-keys` and its `[id]/rotate`; `POST /api/v1/admin/oauth-clients`, its `PATCH [id]` and `[id]/rotate-secret`; `PATCH /api/administrator/mcp-agents/[id]`; `POST /api/v1/me/api-keys` and, for a bearer caller, its `[id]/rotate`. `tests/unit/credential-issuance-invariant.test.ts` discovers issuing calls by scanning and fails the build when one sits outside a handler that runs the rule. A refusal is `invalid_scope` with the offending `ungrantableScopes` (403 on a rotation, audited as `*.rotate_denied` / `scope_not_grantable`); a rotation of an inactive credential still answers `409` first.
