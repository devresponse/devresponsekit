...
4. **Use a direct (non-pooled) endpoint** in `DATABASE_URL` by default — the app sets `search_path` via a startup parameter a transaction-pooling pooler **rejects** (`08P01`). To use a **pooled** endpoint on serverless, set `DB_SEARCH_PATH_VIA_OPTIONS=0` **and** make the schema a role default — `ALTER ROLE <app_role> SET search_path = auth, public;` (see [Configuration](./configuration.md#database-postgresql)). Tune `PGPOOL_MAX` and the timeouts for your host. Migrations/seeds/reset always use the direct endpoint.
