...
| i18n | next-intl, `*.errors.*` namespaces; **no** dedicated `validation.*` namespace. Locale parity enforced across `en/fr/es/uk/pt`. |
...
`passwordMin`, `passwordsMismatch`, `slug`, `key`, …) in **en/fr/es/uk/pt**,
...
- **Locale parity** stays green — validation keys added to all five locales in
