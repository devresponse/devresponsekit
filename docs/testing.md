...
| [`ci.yml`](../.github/workflows/ci.yml) | `Deploy CLI (drk-deploy)` | **operator step**: add it (see below) | pull_request, push to `main` |
...
`Deploy CLI (drk-deploy)` (F-45) installs, typechecks, builds, tests and format-checks the deploy CLI in [`vercel-cli/`](../vercel-cli/README.md#ci), which is its own pnpm package that the kit's typecheck, lint and format skip. It is the only tool that applies production migrations before it promotes a build, and before this job nothing in CI built it, so a Dependabot bump that broke it, or a refactor that promoted before migrating, merged green. Its suite asserts that release order through an injected runner and never reaches Vercel or a database. The job runs on every pull request, with no path filter: a required check skipped by a path filter never reports, and GitHub leaves it pending, which would block every merge. **Operator step:** add `Deploy CLI (drk-deploy)` to the required status checks of `main`'s branch protection. Until then a red run is visible but does not block a merge. The job has no audit step: the `Dependency audit` check above already audits `vercel-cli/pnpm-lock.yaml`. `tests/unit/vercel-cli-ci.test.ts` pins the job's name, its triggers, and its install, typecheck, test and format steps. `tests/unit/drk-deploy-required-keys.test.ts` checks the CLI's required environment keys against `src/lib/env.ts`, in the kit's own suite.
