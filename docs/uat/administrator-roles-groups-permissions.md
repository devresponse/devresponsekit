...
- UAT-ADMIN-RGP-GROUPS-LIST-S2 — As an Org Admin, I want to delete a group, so that I can remove a cohort; deletion should not strip roles that users hold directly.
  - Acceptance criteria: Given `admin.groups.delete`, when I confirm delete, then the group and its role bundles + memberships cascade away, but a member keeps any role assigned to them directly. (There is no in-use block on group delete, but it carries the REVOKE-1 conferral guard: a group that confers any permission I do not hold is refused, see the negative cases below.)
  - UAT script:
    | # | Step | Expected result |
    |---|---|---|
...
- Group delete of a group that confers a permission the actor does not hold (for example `superuser`, or an admin permission outside their own role) → **403**, the inline "You do not have permission to perform this action." error, the row stays, and an `admin.group.delete_denied` audit row (reason `unheld_permissions`) records the attempt (REVOKE-1, F-11). A Superadmin deletes it normally.
- Any other group delete failure → inline generic error (`_groups-grid.tsx`).
