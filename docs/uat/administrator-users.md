...
- ADMIN-USERS-NEW-S1 — As an Org Admin, I want to create a user, so that I can provision access on someone's behalf.
  ...
  - UAT script:
    ...
    | 5 | Open the **Memberships** tab. | One membership, in ORG A (`org-a`, the org you act in), with status **Pending approval**: the create enrolled the user there, so the detail page loads for an Org Admin instead of answering 404. |
  - Result: [ ] Pass  [ ] Fail  — Notes: ______
