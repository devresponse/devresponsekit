...
7. Containing the admin ends the session they are impersonating from (F-08) → in browser A, sign in as `orgadmin@orga.local` and impersonate `user1@orga.local`. In browser B, sign in as `superuser@orga.local`, open `orgadmin@orga.local`'s detail and click **Revoke all** on the Sessions tab. Banning or deleting `orgadmin` from the Users list's bulk actions, or setting their password with `POST …/password` (`mode: "set"`), behaves the same for browser A. Setting the password ends only the borrowed session, though: any browser signed in as `orgadmin` themselves stays signed in (F-10, open). Reload browser A: it lands on sign-in, not on `user1`'s dashboard. The borrowed session belonged to `user1`, so it never appeared on `orgadmin`'s Sessions tab, and before F-08 it survived every one of those actions (`src/lib/impersonation-sessions.server.ts`). A completed password reset for `orgadmin` ends it too.
