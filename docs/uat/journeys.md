...
- **Preconditions:** app seeded (the three placeholder enterprise apps exist — `src/db/seeds/seed-local.ts:133-164`); the SSO handoff env is configured. To exercise the full cross-subdomain hop you need a satellite origin that also runs this kit and points at this primary — `SSO_HANDOFF_ISSUER` (the primary's origin, whose `/api/sso/jwks.json` the satellite verifies against; no shared secret), `SSO_HANDOFF_AUDIENCE_PREFIX`, `SSO_HANDOFF_APPLICATION_ID` (`src/app/[locale]/(auth)/sso/confirm/page.tsx:25-27`, `src/lib/jwt-handoff.server.ts`) — and the primary needs `SSO_HANDOFF_PRIVATE_KEY`. `TODO: verify` the satellite/env is provisioned in your test setup; without it, step 3 shows the invalid-token screen.
- **Code:** the hub is the **Applications** sheet in the top shell bar, rendering each app as an anchor to its `ssoLaunchUrl` = `/api/sso/launch?...` (`src/components/app-shell/application-switcher-sheet.tsx:31-33,135-140`); launch mints a short-lived one-time EdDSA JWT and redirects to the target's `/api/sso/consume?token=…` (`src/app/api/sso/launch/route.ts:45-63`, `src/lib/sso.server.ts`); the consume GET verifies the token and redirects to `/sso/confirm`, which re-verifies the token to show the account and requires a same-origin POST back to `/api/sso/consume` (`src/app/[locale]/(auth)/sso/confirm/page.tsx:8-22,78-90`).
...
- **Personas:** **Superadmin** (steps 1-3). The template catalog is platform-global, so both the Edit link and the edit page are superadmin-only (review #73); an Org Admin holding `admin.email.manage` can still run steps 4-6 (the outbox test send).
- **Preconditions:** app seeded (template rows exist per key and locale).
- **Code:** templates list guard `admin.email.read`, edit link per row shown only to a superadmin (`src/app/[locale]/(secure)/app/administrator/email/templates/page.tsx:50,58,145`); edit page guard `admin.email.manage` **plus** `isSuperadmin` → `notFound()` (`src/app/[locale]/(secure)/app/administrator/email/templates/[templateId]/page.tsx:33,39`), with `key` and `locale` shown but **immutable** — each `(key, locale)` is its own row (`src/app/[locale]/(secure)/app/administrator/email/templates/[templateId]/page.tsx:17-18,60-66`); save is `PUT /api/administrator/email/templates/[id]` (`.../[templateId]/_template-edit-form.tsx:64`); send-test lives in the outbox toolbar and posts to `/api/administrator/email/test` (`src/app/[locale]/(secure)/app/administrator/email/_outbox-grid.tsx:204-251`), which sends the fixed `test_email` template through the outbox and requires `admin.email.manage` (`src/app/api/administrator/email/test/route.ts:32,65`); outbox row detail renders the stored subject/body (`.../email/_outbox-grid.tsx:263-305`).
...
| # | Step (what to do) | Expected result |
|---|---|---|
| 1 | As a **Superadmin**, open Administrator → **Email** → **Templates**. | A table lists template rows by **key** and **locale** (one row per key per locale), each with an **Edit** link. (As an Org Admin the same table loads with no Edit links, and the edit URL is 404.) |
...
- **Screens:** Account → **Preferences** (change locale) → the page moves to the new locale and the shell follows → trigger an email → (admin) Email **outbox** shows the next email localized.
...
- **Code:** preferences page + locale selector (`src/app/[locale]/(secure)/app/account/preferences/page.tsx:40-49`, form `.../preferences/_preferences-form.tsx:167-185`); save is `PUT /api/account/preferences`, then, when the language changed, a move to the same page under the new locale (`useSwitchLocale`, F-37; `.../preferences/_preferences-form.tsx:133-150`), else `router.refresh()`; the API mirrors the choice onto `app_users.preferred_locale` (`src/app/api/account/preferences/route.ts:89-95`), which the email sender reads but the UI does not (the URL segment is the request locale); the next email resolves the recipient's locale from `app_users.preferred_locale` and picks the matching template row (falling back to `en`) (`src/lib/email/send.server.ts:111-122`, template resolution `:61-82`); supported locales `src/config/i18n-config.ts:10`.
...
| 2 | Change the language to a non-Latin locale — e.g. **Ukrainian** (`uk`) or **Japanese** (`ja`) — and save. | The address changes to `/uk/…` (or `/ja/…`) and the page re-renders in that language. |
