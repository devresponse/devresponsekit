/* tslint:disable */
/* eslint-disable */
/**
 * DevResponse Administrator API
 * Administrator console API (`/api/administrator`). Authenticate EITHER with the Better Auth **session cookie** (send credentials with the request; every **mutation** then also requires an `Origin` or `Referer` header matching a trusted origin — the CSRF guard — which a browser sets itself and a server-side caller must add) OR with a **bearer** API key / JWT, whose effective authority is the intersection of its scopes and the permissions of its owner and which is exempt from the origin guard. The one exception is `DELETE /users/{id}/impersonate`, which bypasses the permission guard by design and is therefore **cookie-session only** (see that operation). Org admins are scoped to their own organization (out-of-scope resources return 404, not 403). Errors use `{ error, message, requestId }`; every response carries an `x-request-id` header.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { EmailTemplate } from './EmailTemplate';
import {
    EmailTemplateFromJSON,
    EmailTemplateFromJSONTyped,
    EmailTemplateToJSON,
    EmailTemplateToJSONTyped,
} from './EmailTemplate';

/**
 * 
 * @export
 * @interface EmailTemplateListEnvelope
 */
export interface EmailTemplateListEnvelope {
    /**
     * 
     * @type {Array<EmailTemplate>}
     * @memberof EmailTemplateListEnvelope
     */
    items: Array<EmailTemplate>;
}

/**
 * Check if a given object implements the EmailTemplateListEnvelope interface.
 */
export function instanceOfEmailTemplateListEnvelope(value: object): value is EmailTemplateListEnvelope {
    if (!('items' in value) || value['items'] === undefined) return false;
    return true;
}

export function EmailTemplateListEnvelopeFromJSON(json: any): EmailTemplateListEnvelope {
    return EmailTemplateListEnvelopeFromJSONTyped(json, false);
}

export function EmailTemplateListEnvelopeFromJSONTyped(json: any, ignoreDiscriminator: boolean): EmailTemplateListEnvelope {
    if (json == null) {
        return json;
    }
    return {
        
        'items': ((json['items'] as Array<any>).map(EmailTemplateFromJSON)),
    };
}

export function EmailTemplateListEnvelopeToJSON(json: any): EmailTemplateListEnvelope {
    return EmailTemplateListEnvelopeToJSONTyped(json, false);
}

export function EmailTemplateListEnvelopeToJSONTyped(value?: EmailTemplateListEnvelope | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'items': ((value['items'] as Array<any>).map(EmailTemplateToJSON)),
    };
}

