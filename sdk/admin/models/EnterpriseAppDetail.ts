/* tslint:disable */
/* eslint-disable */
/**
 * DevResponse Administrator API
 * Administrator console API (`/api/administrator`). Authenticate EITHER with the Better Auth **session cookie** (send credentials with the request; every **mutation** then also requires an `Origin` or `Referer` header matching a trusted origin — the CSRF guard — which a browser sets itself and a server-side caller must add) OR with a **bearer** API key / JWT, whose effective authority is the intersection of its scopes and the permissions of its owner and which is exempt from the origin guard. The one exception is `DELETE /users/{id}/impersonate`, which bypasses the permission guard by design and is therefore **cookie-session only** (see that operation). Org admins are scoped to their own organization (out-of-scope resources return 404, not 403). Errors use `{ error, message, requestId }`; every response carries an `x-request-id` header.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface EnterpriseAppDetail
 */
export interface EnterpriseAppDetail {
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    id: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    label: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    description?: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    origin: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    subdomain: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    ssoAudience: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    status?: string;
    /**
     * 
     * @type {number}
     * @memberof EnterpriseAppDetail
     */
    sortOrder?: number;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    organizationId?: string;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    organizationSlug?: string;
    /**
     * 
     * @type {Date}
     * @memberof EnterpriseAppDetail
     */
    createdAt?: Date;
    /**
     * 
     * @type {string}
     * @memberof EnterpriseAppDetail
     */
    organizationName?: string | null;
}

/**
 * Check if a given object implements the EnterpriseAppDetail interface.
 */
export function instanceOfEnterpriseAppDetail(value: object): value is EnterpriseAppDetail {
    if (!('id' in value) || value['id'] === undefined) return false;
    if (!('label' in value) || value['label'] === undefined) return false;
    if (!('origin' in value) || value['origin'] === undefined) return false;
    if (!('subdomain' in value) || value['subdomain'] === undefined) return false;
    if (!('ssoAudience' in value) || value['ssoAudience'] === undefined) return false;
    return true;
}

export function EnterpriseAppDetailFromJSON(json: any): EnterpriseAppDetail {
    return EnterpriseAppDetailFromJSONTyped(json, false);
}

export function EnterpriseAppDetailFromJSONTyped(json: any, ignoreDiscriminator: boolean): EnterpriseAppDetail {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'],
        'label': json['label'],
        'description': json['description'] == null ? undefined : json['description'],
        'origin': json['origin'],
        'subdomain': json['subdomain'],
        'ssoAudience': json['sso_audience'],
        'status': json['status'] == null ? undefined : json['status'],
        'sortOrder': json['sort_order'] == null ? undefined : json['sort_order'],
        'organizationId': json['organization_id'] == null ? undefined : json['organization_id'],
        'organizationSlug': json['organization_slug'] == null ? undefined : json['organization_slug'],
        'createdAt': json['created_at'] == null ? undefined : (new Date(json['created_at'])),
        'organizationName': json['organization_name'] == null ? undefined : json['organization_name'],
    };
}

export function EnterpriseAppDetailToJSON(json: any): EnterpriseAppDetail {
    return EnterpriseAppDetailToJSONTyped(json, false);
}

export function EnterpriseAppDetailToJSONTyped(value?: EnterpriseAppDetail | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'id': value['id'],
        'label': value['label'],
        'description': value['description'],
        'origin': value['origin'],
        'subdomain': value['subdomain'],
        'sso_audience': value['ssoAudience'],
        'status': value['status'],
        'sort_order': value['sortOrder'],
        'organization_id': value['organizationId'],
        'organization_slug': value['organizationSlug'],
        'created_at': value['createdAt'] == null ? undefined : ((value['createdAt']).toISOString()),
        'organization_name': value['organizationName'],
    };
}

