/* tslint:disable */
/* eslint-disable */
/**
 * DevResponse Administrator API
 * Administrator console API (`/api/administrator`). Authenticate EITHER with the Better Auth **session cookie** (send credentials with the request; every **mutation** then also requires an `Origin` or `Referer` header matching a trusted origin — the CSRF guard — which a browser sets itself and a server-side caller must add) OR with a **bearer** API key / JWT, whose effective authority is the intersection of its scopes and the permissions of its owner and which is exempt from the origin guard. Org admins are scoped to their own organization (out-of-scope resources return 404, not 403). Errors use `{ error, message, requestId }`; every response carries an `x-request-id` header.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface UpdateEmailTemplateRequest
 */
export interface UpdateEmailTemplateRequest {
    /**
     * 
     * @type {string}
     * @memberof UpdateEmailTemplateRequest
     */
    subject: string;
    /**
     * 
     * @type {string}
     * @memberof UpdateEmailTemplateRequest
     */
    bodyHtml: string;
    /**
     * 
     * @type {string}
     * @memberof UpdateEmailTemplateRequest
     */
    bodyText?: string | null;
    /**
     * 
     * @type {string}
     * @memberof UpdateEmailTemplateRequest
     */
    description?: string | null;
}

/**
 * Check if a given object implements the UpdateEmailTemplateRequest interface.
 */
export function instanceOfUpdateEmailTemplateRequest(value: object): value is UpdateEmailTemplateRequest {
    if (!('subject' in value) || value['subject'] === undefined) return false;
    if (!('bodyHtml' in value) || value['bodyHtml'] === undefined) return false;
    return true;
}

export function UpdateEmailTemplateRequestFromJSON(json: any): UpdateEmailTemplateRequest {
    return UpdateEmailTemplateRequestFromJSONTyped(json, false);
}

export function UpdateEmailTemplateRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): UpdateEmailTemplateRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'subject': json['subject'],
        'bodyHtml': json['body_html'],
        'bodyText': json['body_text'] == null ? undefined : json['body_text'],
        'description': json['description'] == null ? undefined : json['description'],
    };
}

export function UpdateEmailTemplateRequestToJSON(json: any): UpdateEmailTemplateRequest {
    return UpdateEmailTemplateRequestToJSONTyped(json, false);
}

export function UpdateEmailTemplateRequestToJSONTyped(value?: UpdateEmailTemplateRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'subject': value['subject'],
        'body_html': value['bodyHtml'],
        'body_text': value['bodyText'],
        'description': value['description'],
    };
}

