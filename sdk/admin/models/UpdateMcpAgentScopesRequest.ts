/* tslint:disable */
/* eslint-disable */
/**
 * DevResponse Administrator API
 * Administrator console API (`/api/administrator`). Authenticate EITHER with the Better Auth **session cookie** (send credentials with the request; every **mutation** then also requires an `Origin` or `Referer` header matching a trusted origin — the CSRF guard — which a browser sets itself and a server-side caller must add) OR with a **bearer** API key / JWT, whose effective authority is the intersection of its scopes and the permissions of its owner and which is exempt from the origin guard. The one exception is `DELETE /users/{id}/impersonate`, which bypasses the permission guard by design and is therefore **cookie-session only** (see that operation). Org admins are scoped to their own organization (out-of-scope resources return 404, not 403). Errors use `{ error, message, requestId }`; every response carries an `x-request-id` header.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface UpdateMcpAgentScopesRequest
 */
export interface UpdateMcpAgentScopesRequest {
    [key: string]: any | any;
    /**
     * 
     * @type {Array<string>}
     * @memberof UpdateMcpAgentScopesRequest
     */
    scopes: Array<string>;
}

/**
 * Check if a given object implements the UpdateMcpAgentScopesRequest interface.
 */
export function instanceOfUpdateMcpAgentScopesRequest(value: object): value is UpdateMcpAgentScopesRequest {
    if (!('scopes' in value) || value['scopes'] === undefined) return false;
    return true;
}

export function UpdateMcpAgentScopesRequestFromJSON(json: any): UpdateMcpAgentScopesRequest {
    return UpdateMcpAgentScopesRequestFromJSONTyped(json, false);
}

export function UpdateMcpAgentScopesRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): UpdateMcpAgentScopesRequest {
    if (json == null) {
        return json;
    }
    return {
        
            ...json,
        'scopes': json['scopes'],
    };
}

export function UpdateMcpAgentScopesRequestToJSON(json: any): UpdateMcpAgentScopesRequest {
    return UpdateMcpAgentScopesRequestToJSONTyped(json, false);
}

export function UpdateMcpAgentScopesRequestToJSONTyped(value?: UpdateMcpAgentScopesRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
            ...value,
        'scopes': value['scopes'],
    };
}

