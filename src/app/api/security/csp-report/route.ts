 ...
 * token-bucket floor before any work — per IP first, so a report its IP
 * bucket refused never spends the global budget and one flooding IP cannot
 * silence every real report deployment-wide (F-18); consumed from the SHARED
 * Postgres bucket (review #98), because an in-memory floor on a per-lambda
 * runtime multiplied by the invocation count and was global in name only;
 * when the database is unreachable the floor falls back to the in-process
 * bucket with a warning rather than dropping or 5xx-ing reports — (2) processes at most
...
export async function POST(request: Request): Promise<NextResponse> {
  // Flood floor FIRST (P2-5): cap request volume per IP, then globally, before
  // any parsing or logging. The global floor is charged only for a report its
  // IP bucket admitted (F-18). Always 204 so a throttled probe learns nothing.
  const floorCheck = await consumeSourceThenGlobal("csp.report", clientIpKey(request.headers), {
    source: CSP_IP_LIMIT,
    global: CSP_GLOBAL_LIMIT,
  });
