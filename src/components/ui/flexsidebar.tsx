 ...
 *
 * This variant keeps the ORIGINAL `SidebarProvider` / context / state
 * machine (cookie persistence, Ctrl/Cmd+B, mobile sheet) and changes
 * only the layout DOM:
 *   - the provider wrapper fills its parent (`h-full min-h-0`) instead
 *     of the viewport;
 *   - the desktop sidebar is ONE in-flow flex column whose own width
 ...
 * sub-component (menu, tooltip-on-collapsed, rail, inset) works
 * unchanged.
 */
  ...
  ({ className, ...props }, ref) => {
    return (
      <main
        ref={ref}
        className={cn(
          // min-h-0 / min-w-0 let the inset shrink with the parent
          // instead of forcing the flex row to grow past it.
          "bg-background relative flex h-full min-h-0 w-full min-w-0 flex-1 flex-col",
