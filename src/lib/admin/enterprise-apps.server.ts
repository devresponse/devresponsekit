...
/**
 * Shared validation helpers for the enterprise-apps endpoints
 * (docs/admin-manager.md §8.10, Phase 6).
 *
 * Centralized so the create-route, the update-route, and the client
 * form all enforce the same rules — and so the rules live in exactly
 * one place when we tighten them.
 */
...
/**
 * SSO audience identifier; `audience-prefix:app-id` style. We accept
 * the same character class as the app id but allow ASCII colons so
 * the conventional `prefix:suffix` shape works.
 */
export const SSO_AUDIENCE_RE = /^[a-z0-9](?:[a-z0-9._:-]{0,191})$/;
