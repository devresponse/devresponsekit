...
export async function dbProvision(
  ...
  info(`Then migrate with:  ${bold("drk-deploy migrate --database-url <direct-url>")}`);
}
...
export async function dbStatus(cliRoot: string): Promise<void> {
  ...
  if (dbVars.length === 0) {
    ...
    info(`Provision one with ${bold("drk-deploy db:provision")}, or set DATABASE_URL yourself.`);
    return;
  }
